...
if no_cover:
    ...
    print('    - drop the image at covers/full/<id>.jpg (any format), then run')
    print('      python tools/avif_convert.py --adopt')
    print('    - or add a cover_url for that id in ../tsr_products/covers.csv,')
    print('      run download_covers.py <year>, then --adopt as above.')
    print('  Then generate_thumbs.py and re-run this script.')
    print('  A cover_url left blank on purpose (upstream scan rejected) will keep')
    print('  reappearing here. That is expected, not a task.')
    print('=' * 68)
