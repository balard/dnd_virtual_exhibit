...
- **hotsite.html** — **Isolated one-off promo page; treat as out of scope.** A trimmed `spread.html` showing a hardcoded five-product set (`HOTSITE_IDS = [55, 36, 151, 1993, 77]`, in that order) as back+front spreads: clamped navigation with no wrap, no Random, no autoplay, Back returns to plain `index.html`, Details panel kept. Nothing in the app links to it — that is deliberate, and it carries `noindex, nofollow`.
  ...
  - Its only real coupling is `products.json`: it needs those five ids, their `cover_url` and `backcover_url`. All five currently have both covers, so it always renders a true two-page spread. A data change that dropped one of those ids would silently shorten the set (missing ids are skipped, and the page only errors if all five vanish) — the one failure worth knowing about, but not a reason to check it routinely.
