...
- **covers/cover_colors.json** — Generated sidecar keyed by product id: `{ "<id>": {"avg": "#rrggbb", "dom": "#rrggbb", "vib": "#rrggbb"} }` (`avg` = overall average color, `dom` = dominant swatch, `vib` = most saturated swatch of the cover's central art with the frame cropped out); consumed by `stats.html` to color the settings/artists bars
